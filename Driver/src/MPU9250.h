...
#define MPU9255_Address 0x68 //cs		 Адрес для I2C
...
volatile unsigned long time_interrupt_9250;						 //Переменная для запоминания Времени в которое были готовы данные по прерыванию
...
{
	Serial.print("Angle -> ");
	Serial.print(angleCompX2, 2);
	Serial.print(" ");
	Serial.print(angleCompX, 2);
	Serial.print(" ");
	Serial.print(kalmAngleX, 2);
	Serial.print(" ");
	Serial.print(angleAccX, 1);
	Serial.print(" ");
	Serial.print(angleGyroY, 1);
	Serial.print(" ");
...
void Init_9255()
{
	...
	set_TCA9548A(multi_line_MPU9250); //Переключаем мультиплексор

	// pinMode(PIN_MPU9250_Adr, OUTPUT);
	// digitalWrite(PIN_MPU9250_Adr, 1); // Подаем 1 что-бы адрес был всегда один и тоже 0x69
	//I2C_test();
	...
	Serial.print("WIA_MPU: ");
	Serial.println(WIA_MPU, HEX);
	...
	if ((WIA_MPU == 0x68) | (WIA_MPU == 0x71) | (WIA_MPU == 0x73)) //71- 9050       73-9255
	{
		...
		if (WIA_MPU == 0x73)
			Serial.println(" Successfully connected to MPU9255");
		...
		WriteByte_I2C(MPU9255_Address, 0x19, 19); // устанавливаем HEX формате 7 в BIN (111) и получаем частоту 1 раз в 8 милисекунды или 125 Герц
		...
		WriteByte_I2C(MPU9255_Address, 0x1B, 0b00000000);
		gyro_delitel = 131; //  250dps     // 27 регистр GYRO_CONFIG Устанавливаем чувствительность Гироскопа
...
double getCompAngleZ(float magD, float comp_GyroZ, float dt, float coefficient_) //Простейший комплементарный фильтр
{
	//a(t) = (1-K) * (a(t-1) + gx*dt) + K * acc
	...
	Z_angle_comp = ((1 - coefficient_) * gyroD) + (coefficient_ * magD);

	//Serial.print(" Z1= ");	Serial.print(Z_angle_comp);

	if (Z_angle_comp < 0)
	{
		Z_angle_comp = Z_angle_comp + 360;
	}
	if (Z_angle_comp > 360)
	{
		Z_angle_comp = Z_angle_comp - 360;
...
void Read_9255_I2C() // Указываем на каком пине устройство и с какого регистра нужно прочитать данные
{
	...
	if (reza != 0)
	{
		Serial.print("!!! 9255 Write Mistake reza = ");
		Serial.println(reza);
		Serial2.print("!!! 9255 Write Mistake reza = ");
		Serial2.println(reza);
	};

	// uint8_t buffer_acc_gyroTemp[14];			 // Буфер для считывания данных аксельрометра  временный
	int count_byte = 14; // число запрашиваемых байт
	...
	{
		Serial.print("!!! 9255 Write Mistake rezb = ");
		Serial.println(rezb);
		Serial2.print("!!! 9255 Write Mistake rezb = ");
		Serial2.println(rezb);
	}
...
void Update_9255()
{
	//Вычисляем время которео прошло с прошлой обработки данных
	intervalAccGyro = (time_interrupt_9250 - pre_intervalAccGyro) * 0.000001; // Измеряем время прошедшее с предудущего считывания данных гироскопа в секундах
	pre_intervalAccGyro = time_interrupt_9250;
	
	//Вычисляем углы
	angleAccX = atan2(accX, accZ) * RAD_TO_DEG;
	angleAccY = atan2(accY, accZ) * RAD_TO_DEG;

	// Serial.print(" angleAccX = ");
	// Serial.print(angleAccX);
	// Serial.print(" angleAccY = ");
	// Serial.print(angleAccY);
...
*/
	...
	angleCompX = getComplementarnX(angleAccX, angleGyro_Delta_Y, 0.01);
	angleCompY = getComplementarnY(angleAccY, angleGyro_Delta_X, 0.01);
	angleCompZ = getCompAngleZ(magD, gyroZ, intervalAccGyro, 0.01);
	...
	// Serial.print(" angleCompX = ");
	// Serial.print(angleCompX);
	// Serial.print(" angleCompY = ");
	// Serial.print(angleCompY);
	// Serial.print(" angleCompZ = ");
	// Serial.print(angleCompZ);
}

void ReadCalibrovka_AccGyro()
{
	if (eeprom_read_byte((byte *)32) == 1) // Если есть поправочные значения то считываем их
	{
		Serial.println(" Read Colibrovka from EEPROM ");
		accXoffset = eeprom_read_float((float *)34);
		accYoffset = eeprom_read_float((float *)38);
		accZoffset = eeprom_read_float((float *)42);
		gyroXoffset = eeprom_read_float((float *)46);
		gyroYoffset = eeprom_read_float((float *)50);
		gyroZoffset = eeprom_read_float((float *)54);
		Serial.print("AX : ");
		Serial.println(accXoffset, 6);
		Serial.print("AY : ");
		Serial.println(accYoffset, 6);
		Serial.print("AZ : ");
		Serial.println(accZoffset, 6);
		Serial.print("GX : ");
...
void Calibrovka_AccGyro()
{
	...
	eeprom_write_byte((byte *)32, 1); // записываем признак что есть данные колибровки в 10 адрес
	Serial.print("Flag ACC_GYRO: ");
	Serial.println(eeprom_read_byte((byte *)32));

	Serial.println("==================ACC======================");

	Serial.print("X : ");
	Serial.println(accXoffset, 6);
	eeprom_write_float((float *)34, accXoffset);
	Serial.print("accX offset : ");
	Serial.println(eeprom_read_float((float *)34), 6);

	Serial.print("Y : ");
	Serial.println(accYoffset, 6);
	eeprom_write_float((float *)38, accYoffset);
	Serial.print("accY offset : ");
	Serial.println(eeprom_read_float((float *)38), 6);

	Serial.print("Z : ");
	Serial.println(accZoffset, 6);
	eeprom_write_float((float *)42, accZoffset);
	Serial.print("accZ offset : ");
	Serial.println(eeprom_read_float((float *)42), 6);

	Serial.println("==================GYRO=====================");
	Serial.print("X : ");
	Serial.println(gyroXoffset, 6);
	eeprom_write_float((float *)46, gyroXoffset);
	Serial.print("gyroX offset : ");
	Serial.println(eeprom_read_float((float *)46), 6);

	Serial.print("Y : ");
	Serial.println(gyroYoffset, 6);
	eeprom_write_float((float *)50, gyroYoffset);
	Serial.print("gyroY offset : ");
	Serial.println(eeprom_read_float((float *)50), 6);

	Serial.print("Z : ");
	Serial.println(gyroZoffset, 6);
	eeprom_write_float((float *)54, gyroZoffset);
	Serial.print("gyroZ offset : ");
	Serial.println(eeprom_read_float((float *)54), 6);
