...
// Одометрия
float speed_L_fact; // Скорость на моторе фактически установленная m/s
float speed_R_fact; // Скорость на моторе фактически установленная m/s
...
// Проверка и ограничение минимальной частоты вращения. Если меньше минимума то считаем что ноль
float minRPS(float _rps)
{
    if (_rps > 0 && _rps < 0.001)
        _rps = 0;
    if (_rps < 0 && _rps > -0.001)
        _rps = 0;
    return _rps;
...
{
    // Serial.print(" Timeing_Step_L rps_= ");
    // Serial.println(rps_);
    rps_L_fact = rps_; // Фиксируем фактичские обороты
...
{
    _rps = minRPS(_rps);                    // Проверка минимального rps
    float rps_start = ACCELERATION / 100.0; // Начальные обороты с которых стартуем
    if (rps_L_fact > rps_start)             // Усли уже есть обороты на которых едем
    {
...
void setSpeedRPS_R(float _rps)
{
    _rps = minRPS(_rps);                    // Проверка минимального rps
    float rps_start = ACCELERATION / 100.0; // Начальные обороты с которых стартуем
    if (rps_R_fact > rps_start)             // Усли уже есть обороты на которых едем
    {
...
void getKoefAccel()
{
    ...
    Serial.print(d_gived_R);
    if (d_gived_L != 0 || d_gived_R != 0) // Если хоть одна разница скоротей не равна нулю
    {
        float k = (d_gived_L + d_gived_R) / 2; // Для ускорения при движении по огружности находим среднюю скорость в оборотах. 9-1 =10/2=5
        float kL = d_gived_L / k;              // Ускорение на левом моторе 9/5=1.8
        float kR = d_gived_R / k;              // Ускорение на левом моторе 1/5=0.2
        ...
        float kkk = 0; // Отношение ускорений которое надо получить
        if (kL >= kR)
        {
            kkk = kL / kR;
            // acceler_R = kR / (kL / ACCELERATION); //
            acceler_R = 1 / kkk; //
        }
        else
        {
            kkk = kR / kL;
            // acceler_L = kL / (kR / ACCELERATION); //
            acceler_L = 1 / kkk; //
        }
        Serial.print(" kkk= ");
        Serial.print(kkk);
        // float ku = 1;
        // if (maxK > ACCELERATION) // Если получилось что ускорение превышает макисмальное то считаем коэффициент уменьшения
        // {
        //     ku = maxK / ACCELERATION; // находим насколько ускорение на внешнем моторе больше максимального ускорения 1.8 /1.2 = 1.5
        // }

        // acceler_L = kL / ku; // 1,8/1,5=1,2
        // acceler_R = kR / ku; // 0,2/1,5=0,13333

        // float maxK = 0;
        // if (d_gived_L >= d_gived_R) // Находим какое больше и коеффициент отношения большего к меньшему
        // {
        //     maxK = d_gived_L / d_gived_R; // 9
        // }
        // else
        // {
        //     maxK = d_gived_R / d_gived_L;
        // }
        // float min_a = ACCELERATION / ((maxK + 1.0) / 2.0); // Находи минимальное ускорение 0,2
        // float max_a = min_a * maxK;                        // Находим максимальное ускорение          1,8

        // if (d_gived_L >= d_gived_R) // Находим какое больше и присваиваем ускорения
        // {
        //     acceler_L = 1;                              //
        //     acceler_R = min_a / (max_a / ACCELERATION); //
        // }
        // else
        // {
        //     acceler_L = min_a / (max_a / ACCELERATION); //
        //     acceler_R = 1;                              //
        // }
    }
...
{
    if (flag_motor_R_EN) // Если мотор работает то и эта функция отрабатывет
    {
        float rps_new = 0;
        float accel = (ACCELERATION * 1) / 100.0; // Ускорение
...
void calculateOdom_enc()
{
    ...
    // printf("dt= %f odom_impuls_R= %i impuls for sec= %f rps= %f \n", dt, odom_impuls_R, odom_impuls_R * k, (gradusR * k) / 360);
...
float checkMaxSpeedRadius(float radius_, float speed_)
{
    ...
    {
        float radius_centre = abs(radius_) - (DISTANCE_WHEELS / 2.0); // Вычисляем дариус по центру машинки, так как просто радиус это внешний радиус по внешнему колесу.
...
{
    g_radius = checkMinRadius(radius_); // Проверка на реалистичность Запоминаяем радиус для расчета одометрии теперь у нас такой радиус отнего все и считаем Это центр робота
    ...
    g_speed = checkMaxSpeedRadius(g_radius, speed_); // Проверка на MAXSPEED Запоминаяем скорость для расчета одометрии теперь у нас такая скорость от нее все и считаем
    ...
    if (g_radius != 0) // Вычисляем коефициент перобразования иначе он остается равен 0
    {
        ...
        k2 = (DISTANCE_WHEELS / 2) / (abs(g_radius) - (DISTANCE_WHEELS / 2)); // Находим долю какую составляем растояние половинки между колесами тела и радиусом поворота.
        // printf("k2= %f \n", k2);
    }
    ...
    {
        setSpeedMS_L(g_speed);
        setSpeedMS_R(g_speed);
    }
