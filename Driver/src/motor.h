...
float getRealRadius(float radius_)
{
    //printf("getRealRadius radius_ IN= %f", radius_);
    //Проверка радиуса на возможность робота
    // float min_r = 0.01;
    // if (radius_ > 0 && radius_ < min_r ) // радиус не может быть меньше минимума за этим нужно следить на верхнем уровне
    // {
    //     radius_ = min_r; // Присваиваем
    // }
    // if (radius_ < 0 && radius_ > -min_r) // радиус не может быть меньше минимума за этим нужно следить на верхнем уровне
    // {
    //     radius_ = -min_r; // Присваиваем
    ...
    // if (radius_ == 0)
    // {
    //     radius_ = 0.001;  // Минимальный радиус. Радиус не может быть
    // }

    return radius_;
}

//Проверка реалистичности скорости заданной учитывая радиус который задан и максимальную скорость моторов с какой они могут вращаться
float getRealSpeed(float radius_, float speed_)
{
    //printf("getRealSpeed speed_ IN= %f", speed_);
    //выставляем скорость
    float k = 0;
    if (radius_ != 0) // Вычисляем коефициент перобразования иначе он остается равен 0
    {
        k = (DISTANCE_WHEELS / 2.0) / radius_; //Находим долю какую составляем растояние половинки между колесами тела и радиусом поворота.
        //printf(" k= %f ", k);
    }
    float have_speed = speed_ * (1 + k); // Скорость с какой хотим ехать по внешнему колесу
    //printf(" have_speed= %f ", have_speed);
    if (have_speed > MAX_SPEED) // Если скорость выходит за максимум тогда ее уменьшаем до возможного максимума
    {
        float k2 = have_speed / MAX_SPEED; // Коеффициент превышения
        //printf(" k2= %f ", k2);
        speed_ = speed_ / k2; // Центр робота по нему считаем одометрию
    }
    // printf(" real= %f \n", speed_);
    return speed_;
}
...
{
    g_radius = getRealRadius(radius_); // Проверка на реалистичность Запоминаяем радиус для расчета одометрии теперь у нас такой радиус отнего все и считаем Это центр робота
    ...
    g_speed = getRealSpeed(abs(g_radius), speed_); // Проверка на реалистичность Запоминаяем скорость для расчета одометриитеперь у нас такая скорость от нее все и считаем
