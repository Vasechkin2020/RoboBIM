...
//  МОЙ РАСЧЕТ КОЛИБРОВКИ под шунт 0,05 Ома и  5 Ампер

void Adafruit_INA219::setCalibration_16V_5A() 
{
  // VBUS_MAX = 16V             (Assumes 32V, can also be set to 16V)                 16
  // VSHUNT_MAX = 0.32          (Assumes Gain 8, 320mV, can also be 0.16, 0.08,
  // 0.04) RSHUNT = 0.05               (Resistor value in ohms)

  // 1. Determine max possible current
  // MaxPossible_I = VSHUNT_MAX / RSHUNT   0.32/0.05 = 6.4
  // MaxPossible_I = 6.4A
  ...
  // MinimumLSB = MaxExpected_I/32767           5/32767 = 0.0001526
  // MinimumLSB = 0.0001526  
  // MaximumLSB = MaxExpected_I/4096            5/4096 = 0.001221
  // MaximumLSB = 0,001221   
  ...
  // 5. Compute the calibration register                                  0.00001
  // Cal = trunc (0.04096 / (Current_LSB * RSHUNT))                  0.04096/(0.0002*0.05) = 4096
  // Cal = 4096 (0x1000)

  ina219_calValue = 4096;
  // 6. Calculate the power LSB
  // PowerLSB = 20 * CurrentLSB      20 * 0.0002 = 0.004
  ...
  ina219_currentDivider_mA = 5; // Current LSB = 100uA per bit (1000/200 = 5)
  ina219_powerMultiplier_mW = 4; // Power LSB = 1mW per bit (2/1)
...
void Adafruit_INA219::setCalibration_32V_2A() {
  ...
  Adafruit_BusIO_Register config_reg =
      Adafruit_BusIO_Register(i2c_dev, INA219_REG_CONFIG, 2, MSBFIRST);
  _success = config_reg.write(config, 2);
  Serial.println("Set 32V and 3.2A");
}
