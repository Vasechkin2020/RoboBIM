...
{
  uint32_t command = 0; // Текущая команда к выполенению 0 - отключить датчики 1 - режим одновременного измерения
};
...
laserStructSend laser[4]; // Все локальные данные по лазерам
...
struct Struct_Modul2Data
{
  ...
  laserStructSend laser[4];  // Структура по состоянию лазеров
  uint32_t newDataLaser = 0; // Статус обновления данных с лазерных датчиков
